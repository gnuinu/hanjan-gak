...
경마 화면은 심야 경마장이다. 관중석·조명탑·전광판·결승 기둥과 말/기수는 전부
인라인 SVG 로 그린다 (`Grandstand.tsx`, `Horse.tsx`). 이미지 파일을 두지 않는 건
오프라인 PWA 라서이기도 하고, 기수 옷을 플레이어 색으로 칠해야 해서이기도 하다.

말의 걸음걸이는 장식이 아니라 상태 표시다. 최근 8프레임 이동거리를 기준 속도로
나눠 배속을 구하고, 그 값이 걸음 주기(`--gallop`)와 자세를 정한다 — 멈추면
고꾸라지고(`is-stumble`), 1.7배가 넘으면 몸을 눕힌다(`is-boost`). 토스트를 놓쳐도
몸짓만 보고 무슨 일이 벌어졌는지 알 수 있다.

9명부터는 이름을 트랙 왼쪽으로 빼서(`race--tight`) 12명이 폰 한 화면에 들어간다.
